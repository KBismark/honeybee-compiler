...
const server = http.createServer(function (req, res) {
    ...
    // Hompage's module
    if (pathname == '/src/modules/index.js') {
...
module.exports = function (base,ignoreMessages) {
    baseDirectory = base;
    i4w_map = require(path.join(baseDirectory,'/i4w.map.js'))()
    server.listen(PORT, ignoreMessages ? () => { }: () => {
        console.log(colors.text(`${colors.text('>>').yellowColor().bold().get()} App started on port ${PORT}`).bold().blueColor().get());
        console.log(colors.text(`Open at ${colors.text(`http://localhost:${PORT}`).cyanColor().get()}`).bold().whiteColor().get());
    })
}