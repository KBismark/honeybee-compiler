var markupPattern =
  ...
  attributePattern =
   /((\$[a-zA-Z0-9-_:()[\]#]+)|([a-zA-Z()[\]#][a-zA-Z0-9-_:()[\]#]*))(?:\s*=\s*((?:'[^']*')|(?:"[^"]*")|(?:{[^]*?})(?=((\s+\S+\s*=)|(\s*(\/>|>))))|\S+))?/gi,
    templatePattern =
    /<>\s*{(\S|\s)*?(?!(?:'[^']*')|(?:"[^"]*")|(?:`[^`]*`))\s*}\s*<\/>|<\$dep\s+value\s*=\s*\{(\S|\s)*?\}\s*>\s*{(\S|\s)*?}\s*<\/\$dep>/g,
...
function parseJsDeps(code) {
  let deps = code.match(/<\$dep\s+value\s*=\s*\{(\S|\s)*?\}/g)[0]
    .replace(/<\$dep\s+value\s*=\s*\{\s*/, '').replace(/(\s*\})$/, ''); //\s*>\s*{(\S|\s)*?}\s*<\/\$dep>
  deps = `(${deps})`;
  code = code.replace(/<\$dep\s+value\s*=\s*\{(\S|\s)*?\}\s*>\s*{/, '')
    .replace(/}\s*<\/\$dep>/, '');
  code = `function(){return UI.CreateDynamicNode(function(state){return (${code})},${deps})}`
...
function parse(html) {
  ...
  var currentNode = createNode(NODES[0]),
    ...
    k = 0,
    l, jsvalue;
  const js = /<>\s*{(\S|\s)*?(?!(?:'[^']*')|(?:"[^"]*")|(?:`[^`]*`))\s*}\s*<\/>/g;
  const jsdeps = /<\$dep\s+value\s*=\s*\{(\S|\s)*?\}\s*>\s*{(\S|\s)*?}\s*<\/\$dep>/g;
  for (i = 1; i < NODES.length - 1; i++) {
    if (typeof NODES[i] !== "string") {
      ...
      while (k < JScripts.length && NODES[i].indexOf(JScripts[k].id) >= 0) {
        ...
        if (NODES[i].length == 1) {
          jsvalue = JScripts[k].value;
          if (js.test(jsvalue)) { 
            currentOpenedNodes[currentOpenedNodes.length - 1].children.push({
              JS: true,
              value: `function(args,state)${jsvalue
                .replace(/<>\s*{/, "{return (")
                .replace(/(}\s*<\/>)$/, ")}")}`,
            });
          } else {
...
function buildString(node, isSVGNamespaceElement, depth, kNdN, eBase, attrSetter) {
  ...
  if (typeof node.attr.key == "string") {
    ...
    var isKeyed = !!keyname.length && /[^ ]/.test(keyname);
  }
  ...
  if (atrris.length > 0) {
        ...
        if (v.startsWith("{") && v.endsWith("}")) {
          ...
          if (/^(on)[A-Z]\S+/.test(k)) {//Events
            ...
            if (k.startsWith("$")) {//Has dependencies
              ...
              if (/^(on)[A-Z]\S+/.test(k)) {//Events
                ...
                if (!leaveAlone.includes(settableK)) {
          ...
          if (!["style", "class"].includes(k)) {
            if (k=="key") {
              ...
              if (!leaveAlone.includes(settableKey)) {
    ...
    if (node.attr.style) {
      node.attr.style = /['`"]/.test(node.attr.style) ? node.attr.style.replace(/['`"]$/, '') : node.attr.style;
      nodeString = nodeString.replace(
        `${attr_Rep}`,
    ...
    for (i = 0; i < ch_l; i++) {
      if (typeof node.children[i] === "string") {
        ...
        if (node.children[i].JS) {
          ...
          attrSetter.dynNodes = `${attrSetter.dynNodes}'${attrSetter.dynCount}':${getNode([...depth, [i,ch_l]],ch_l)},`
...
function getNode(pos, total) {
  ...
  var s = "_$", l = pos.length, last,value;
  for (var i = 0; i < l; i++){
    total = pos[i][1];
    last = total ? total - 1 : NaN;
    switch (value = pos[i][0]) {
      case 0:
        s+=".firstChild"
        break;
      case last:
